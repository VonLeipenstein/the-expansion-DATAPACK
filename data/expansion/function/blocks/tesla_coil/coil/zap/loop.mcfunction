...
execute facing entity @e[tag=exp.zap_target,distance=..12,limit=1,sort=nearest] eyes run function expansion:blocks/tesla_coil/coil/zap/bend
...
execute if entity @s[scores={exp.max_range=..200}] unless entity @e[tag=exp.zapped,distance=..2.1,limit=1,sort=arbitrary] at @s if block ^ ^ ^0.25 #expansion:air run function expansion:blocks/tesla_coil/coil/zap/loop
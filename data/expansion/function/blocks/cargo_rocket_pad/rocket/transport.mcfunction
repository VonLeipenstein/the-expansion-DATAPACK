...
execute if entity @s[tag=exp.returning] at @e[type=minecraft:item_display,tag=exp.cargo_rocket_pad,predicate=expansion:compare_score/pad_id,limit=1] run tp @s ~ ~300 ~
...
function expansion:blocks/portal/teleport/test_valid

# If everything is valid, proceed to the teleportation.
execute if entity @s[tag=exp.valid_portal] run function expansion:blocks/portal/teleport/teleportation_stage_1
tag @s remove exp.valid_portal

# Remove inside components.minecraft:custom_data. This tag is necessary to not make this function run every tick while someone is inside the portal
schedule function expansion:blocks/portal/teleport/schedule_inside_tag 10t

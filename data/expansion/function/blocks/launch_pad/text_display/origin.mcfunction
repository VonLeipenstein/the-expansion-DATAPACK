...
[\
	...
	[\
		...
		[\
			...
			{text:" ≈ "},\
			{score:{name:"#origin",objective:"exp.weight"},color:"gray"}\
		]\
	],\
	[\
		{text:"\n\n"},\
...
execute if dimension minecraft:overworld run data modify storage expansion:temp origin[1] set value {text:"Earth",bold:true,underlined:true,color:"green"}
execute if dimension expansion:moon run data modify storage expansion:temp origin[1] set value {text:"The Moon",bold:true,underlined:true,color:"gray"}
execute if dimension expansion:mars run data modify storage expansion:temp origin[1] set value {text:"Mars",bold:true,underlined:true,color:"red"}
execute if dimension expansion:venus run data modify storage expansion:temp origin[1] set value {text:"Venus",bold:true,underlined:true,color:"gold"}

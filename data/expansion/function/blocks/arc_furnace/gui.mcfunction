...
execute if entity @s[tag=exp.smelting] unless items block ~ ~ ~ container.4 *[custom_data~{gui_item:1b}] run function expansion:utilities/barrel/gui/fill_slot {Slot:4, Type:"arc_furnace_animated"}
execute unless entity @s[tag=exp.smelting] unless items block ~ ~ ~ container.4 *[custom_data~{gui_item:1b}] run function expansion:utilities/barrel/gui/fill_slot {Slot:4, Type:"arc_furnace"}

...
# Callers for expansion:utilities/recipe_predicates/modstation
...
execute if items block ~ ~ ~ container.1 *[custom_data~{exp_item:{name:"steel_ingot"}}] \
        ...
        if items block ~ ~ ~ container.11 *[custom_data~{exp_item:{name:"computing_core"}}] \
        ...
        if items block ~ ~ ~ container.20 *[custom_data~{exp_item:{name:"circuit_board"}}] \

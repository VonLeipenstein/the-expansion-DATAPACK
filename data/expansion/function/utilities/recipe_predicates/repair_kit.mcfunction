execute unless items block ~ ~ ~ container.1 * \
        if items block ~ ~ ~ container.1 *[custom_data~{exp_item:{name:"steel_ingot"}}] \
        ...
        if items block ~ ~ ~ container.11 *[custom_data~{exp_item:{name:"wrench"}}] \
        if items block ~ ~ ~ container.12 slime_ball \
        \

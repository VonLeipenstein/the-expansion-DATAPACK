...
# Callers for expansion:utilities/recipe_predicates/assembler
...
execute if items block ~ ~ ~ container.1 *[custom_data~{exp_item:{name:"steel_ingot"}}] \
        ...
        if items block ~ ~ ~ container.12 *[custom_data~{exp_item:{name:"robot_arm"}}] \
        \
        if items block ~ ~ ~ container.19 *[custom_data~{exp_item:{name:"steel_plate"}}] \
        if items block ~ ~ ~ container.20 *[custom_data~{exp_item:{name:"monochrome_monitor"}}] \

...
# Callers for expansion:utilities/recipe_predicates/electromotor
...
execute if items block ~ ~ ~ container.1 *[custom_data~{exp_item:{name:"steel_ingot"}}] \
        ...
        \
        if items block ~ ~ ~ container.10 *[custom_data~{exp_item:{name:"electromagnet"}}] \
        if items block ~ ~ ~ container.11 *[custom_data~{exp_item:{name:"steel_ingot"}}] \
        if items block ~ ~ ~ container.12 *[custom_data~{exp_item:{name:"electromagnet"}}] \
        \

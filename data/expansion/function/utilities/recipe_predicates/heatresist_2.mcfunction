...
# Callers for expansion:utilities/recipe_predicates/heatresist_2
...
execute if items block ~ ~ ~ container.1 *[custom_data~{exp_item:{name:"amethonium_ingot"}}] \
        ...
        \
        if items block ~ ~ ~ container.10 *[custom_data~{exp_item:{name:"heat_resist_module"},mk:1}] \
        if items block ~ ~ ~ container.11 *[custom_data~{exp_item:{name:"amethonium_plate"}}] \
        if items block ~ ~ ~ container.12 *[custom_data~{exp_item:{name:"cooling_element"}}] \
        \

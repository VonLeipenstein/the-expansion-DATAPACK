...
# Callers for expansion:utilities/recipe_predicates/magnetic_mod
...
execute if items block ~ ~ ~ container.1 *[custom_data~{exp_item:{name:"steel_ingot"}}] \
        if items block ~ ~ ~ container.2 *[custom_data~{exp_item:{name:"electromagnet"}}] \
        ...
        if items block ~ ~ ~ container.20 *[custom_data~{exp_item:{name:"electromagnet"}}] \

# >>> generated function callers >>>
# Callers for expansion:_commands/give/blocks
# Total callers: 0
# None found
# <<< generated function callers <<<

setblock ~ ~ ~ chest{CustomName:{text:"Blocks",color:"#00FFEE"}} replace


loot insert ~ ~ ~ loot expansion:blocks/fabricator
loot insert ~ ~ ~ loot expansion:blocks/oxygenator
loot insert ~ ~ ~ loot expansion:blocks/arc_furnace
loot insert ~ ~ ~ loot expansion:blocks/terraformer
loot insert ~ ~ ~ loot expansion:blocks/lacrymae_extractor
loot insert ~ ~ ~ loot expansion:blocks/planetarium
loot insert ~ ~ ~ loot expansion:blocks/compressor
loot insert ~ ~ ~ loot expansion:blocks/tesla_coil
loot insert ~ ~ ~ loot expansion:blocks/enhancer
loot insert ~ ~ ~ loot expansion:blocks/modstation
loot insert ~ ~ ~ loot expansion:blocks/assembler
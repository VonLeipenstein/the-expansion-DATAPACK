...
execute if biome ~ ~ ~ expansion:moon/ice_lake_caves run return run scoreboard players operation @s exp.temperature = #moon_ice_caves exp.temperature

# all other moon biomes have a temperature that is perfectly balanced by 1 warming module
scoreboard players operation @s exp.temperature = #moon exp.temperature
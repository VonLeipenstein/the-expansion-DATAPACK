...
execute if score @s exp.mech_right_anim matches 0 on passengers if entity @s[type=minecraft:item_display,tag=aj.mech_torso.root] run function animated_java:mech_torso/animations/gatling_shooting_r/play
...
execute if entity @s[tag=exp.mech.action.right.stop] run scoreboard players set @s exp.mech_right_anim 16

...
execute if entity @s[tag=exp.spaceship_pilot] on vehicle run data modify entity @s ArmorItems[3] set from entity @p[tag=exp.spaceship_pilot] Inventory[{Slot:103b}].components.minecraft:custom_data.Inventory[].spaceship_store[]
clear @s[tag=exp.spaceship_pilot] minecraft:stick[custom_data~{spaceship_backup_stick:1b}]

...
execute at @s unless items entity @s weapon.mainhand *[minecraft:custom_data~{exp_item:{name:"fuel_canister"}}] unless predicate expansion:utility/sneak unless entity @s[tag=exp.inside_vehicle] as @e[type=minecraft:interaction,tag=exp.spaceship_rcdet,nbt={interaction:{}},limit=1,sort=nearest] on vehicle if entity @s[tag=exp.spaceship,tag=!exp.being_modified] unless score @s exp.passenger_count matches 2.. run function expansion:vehicles/spaceship/enter/passenger

# fuel
execute at @s if items entity @s weapon.mainhand *[minecraft:custom_data~{exp_item:{name:"fuel_canister"}}] as @e[type=minecraft:interaction,tag=exp.spaceship_rcdet,nbt={interaction:{}},limit=1,sort=nearest] on vehicle run function expansion:vehicles/spaceship/fuel/fill_tank

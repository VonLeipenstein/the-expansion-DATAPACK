...
execute as @e[type=!minecraft:player,tag=!exp.spaceship,tag=!exp.moon_buggy,tag=!exp.zero_gravity,distance=..50,limit=1,sort=nearest] run function expansion:utilities/gravity/zero_gravity/make_float
# summon the ray
summon marker ~ ~1.75 ~ {Tags:[exp.railgun_ray]}
tp @e[type=marker,tag=exp.railgun_ray,limit=1,sort=nearest] @s
execute at @s anchored eyes run tp @e[type=marker,tag=exp.railgun_ray,limit=1,sort=nearest] ^ ^ ^2

# shoot the ray and make sure you can't hit yourself
tag @s add exp.shooting
execute as @e[type=marker,tag=exp.railgun_ray,limit=1,sort=nearest] at @s run function expansion:items/railgun/loop

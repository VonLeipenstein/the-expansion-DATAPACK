...
execute positioned ~ ~-1 ~ if block ~ ~ ~ #expansion:expansion_air run function expansion:items/magnetic_boots/floor_check/check1

# apply the normal zero gravity functions if there are air blocks beneath the player
execute if entity @s[tag=exp.no_floor] run function expansion:utilities/gravity/zero_gravity/zero_gravity

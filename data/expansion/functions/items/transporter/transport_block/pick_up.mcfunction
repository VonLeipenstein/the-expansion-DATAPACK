execute as @e[type=minecraft:armor_stand,tag=exp.position_new,limit=1,sort=random] if score @s exp.unique_id = @e[type=minecraft:armor_stand,tag=exp.transporter,limit=1,sort=random] exp.unique_id at @s as @a[distance=..50] run function expansion:items/transporter/initiate_return

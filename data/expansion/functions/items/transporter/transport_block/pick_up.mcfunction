execute as @e[type=armor_stand,tag=exp.position_new,limit=1,sort=random] if score @s exp.room_id = @e[type=armor_stand,tag=exp.transporter,limit=1,sort=random] exp.room_id at @s as @a[distance=..50] run function expansion:items/transporter/initiate_return

...
# assign unique id
execute unless score @s exp.drone_id matches 1.. run function expansion:items/drone_pet/assign_id
scoreboard players operation @e[type=minecraft:armor_stand,tag=exp.new_drone,limit=1,sort=nearest] exp.drone_id = #global exp.drone_id

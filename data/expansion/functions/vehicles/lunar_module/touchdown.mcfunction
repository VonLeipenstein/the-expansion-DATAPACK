execute as @e[type=armor_stand,tag=exp.lunar_module,limit=1,sort=nearest] at @s run summon minecraft:interaction ~ ~2.5 ~0.7 {Tags:["exp.module_takeoff","exp.module_interactor","smithed.block"],width:1f,height:1f,response:1b}

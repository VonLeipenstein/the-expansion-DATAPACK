...
execute if predicate expansion:dimension/zero_gravity as @e[type=minecraft:armor_stand,tag=exp.moon_buggy,limit=1,sort=nearest] at @s unless block ~ ~-1 ~ #expansion:expansion_air run data merge entity @s {NoGravity:0b}

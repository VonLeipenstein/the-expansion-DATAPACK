...
execute if entity @s[tag=exp.rocket_pilot_rcdet] on vehicle on vehicle on passengers if entity @s[type=armor_stand,tag=exp.rocket_seat] if predicate expansion:passengers/player on passengers run tag @s[type=player] add exp.rocket_pilot

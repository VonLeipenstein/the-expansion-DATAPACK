...
execute at @e[type=item_display,tag=exp.rocket_display,limit=1,sort=nearest] run summon minecraft:armor_stand ~ ~ ~ {Small:1b,Invisible:1b,Invulnerable:1b,Tags:["exp.rocket_seat"]}
ride @e[type=armor_stand,tag=exp.rocket_seat,limit=1,sort=nearest] mount @e[type=item_display,tag=exp.rocket_display,limit=1,sort=nearest]

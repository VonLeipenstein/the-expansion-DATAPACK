...
execute on passengers if entity @s[type=item_display,tag=exp.spaceship_display] store result entity @s item.tag.fuel_lvl int 1 on vehicle run scoreboard players get @s exp.fuel_level
...
execute on passengers if entity @s[type=item_display,tag=exp.spaceship_display] run data modify entity @e[type=item,nbt={Item:{tag:{exp_spaceship:1b}}},limit=1,sort=nearest] Item set from entity @s item

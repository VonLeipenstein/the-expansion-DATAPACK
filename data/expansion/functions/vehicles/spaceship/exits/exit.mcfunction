...
execute at @e[type=minecraft:item_display,tag=exp.spaceship_display,limit=1,sort=nearest] run summon minecraft:interaction ~ ~ ~ {Tags:["exp.spaceship_rcdet","smithed.block"],width:2f,height:2f,response:1b}

# make the item display ride the spaceship and the interaction entity ride the item display
ride @e[type=minecraft:item_display,tag=exp.spaceship_display,distance=..1,limit=1,sort=nearest] mount @e[type=minecraft:armor_stand,tag=exp.spaceship,limit=1,sort=nearest]
ride @e[type=minecraft:interaction,tag=exp.spaceship_rcdet,distance=..1,limit=1,sort=nearest] mount @e[type=minecraft:item_display,tag=exp.spaceship_display,limit=1,sort=nearest]
...
execute as @e[type=minecraft:armor_stand,tag=exp.spaceship,limit=1,sort=nearest] at @s rotated ~ 0 run tp @p[tag=!exp.ejected,tag=exp.inside_spaceship] ^-2 ^ ^1 ~ ~

...
execute if score #temp exp.mech_right_dur matches 0 if score @s exp.mech_right_dur matches 20.. on passengers if entity @s[tag=aj.mech_torso.root] at @s positioned ^-1.3 ^.15 ^3 rotated as @s summon item_display run function expansion:vehicles/mech/actions/righthand/shoot/raycast/cast

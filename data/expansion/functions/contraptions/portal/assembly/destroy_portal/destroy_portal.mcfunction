...
kill @e[type=armor_stand,tag=exp.portal_stand,limit=6,sort=nearest]
kill @e[type=interaction,tag=exp.portal_core_rcdet,limit=5,sort=nearest]
kill @e[type=item_display,tag=exp.portal_display,limit=1,sort=nearest]

fill ^-1 ^-2 ^ ^1 ^-2 ^ minecraft:air replace minecraft:barrier
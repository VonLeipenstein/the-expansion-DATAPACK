...
execute if entity @s[tag=exp.has_ship_selected] unless entity @e[type=minecraft:armor_stand,tag=exp.mod_ship,distance=..50,limit=1,sort=nearest] run function expansion:blocks/modification_station/ship_selection/deselect_ship
...
execute if score @s exp.counter_1 matches ..4 as @e[type=minecraft:armor_stand,tag=exp.mod_drone,limit=5,sort=nearest] run function expansion:blocks/modification_station/mod_robots/main

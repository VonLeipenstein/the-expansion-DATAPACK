...
tag @e[type=armor_stand,tag=exp.active_coil,distance=0.9..10,limit=1,sort=random] add exp.zap_target
execute unless entity @e[type=armor_stand,tag=exp.zap_target,distance=0.9..10,limit=1] run tag @e[type=#expansion:turret_target,tag=!smithed.block,distance=..10,limit=1,sort=nearest] add exp.zap_target

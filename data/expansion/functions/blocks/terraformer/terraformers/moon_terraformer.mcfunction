...
execute if score @s exp.counter_1 matches 0 at @s[scores={exp.counter_1=0}] run spreadplayers ~ ~ 0 40 false @e[type=minecraft:marker,tag=exp.terraformer,limit=1,sort=nearest]
...
execute as @e[type=minecraft:marker,tag=exp.terraformer,limit=1,sort=nearest] at @s run function expansion:blocks/terraformer/terraformers/moon_plant_placer
